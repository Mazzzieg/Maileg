        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if err.resp.status in [500, 503]:
                    ...
                    print(f'Attempt {attempt}/5 failed. Retying in {sleep_time} seconds...')
                    self.logger.warning("Error occurred. Retrying in %s seconds...", sleep_time)
                    time.sleep(sleep_time)
                elif err.resp.status == 403:
                    self.logger.error("A 403 Forbidden error occurred: %s. Please check the API credentials or request parameters.", err)
                    break
                else:
...
class ApiInteraction:
    ...
    def __init__(self, logger, keywords : list, user_mail : str):
        ...
        self.scopes : list = ['https://mail.google.com/','https://www.googleapis.com/auth/calendar' ]
        self.messages : list = []
        self.counter : int = 0
        self.gyms : list = []
        self.dates : list = []
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                flow = InstalledAppFlow.from_client_secrets_file(
                    f'users/{self.user_mail}/{self.user_mail}.json',
                    self.scopes[0] + self.scopes[1])
                creds = flow.run_local_server(port=0)
                with open(token_path, "wb") as token:
                    pickle.dump(creds, token)

        self.gmail_service = (build('gmail', 'v1', credentials=creds)).users().messages() # pylint:disable=maybe-no-member
    ...
    def send_message(self, destination: str, subject: str, body: str) -> None:
        ...
        userId="me",
        body=self.build_message(destination, subject, body)).execute()
    ...
    def reading_answers(self) -> list:
        ...
        try:
            ...
            filtered_body = raw_body.split('>', 1)[0].replace('  ',' ').replace('\n', '')
        ...
        for item in self.list_of_optional_hours:
            try:
                ...
                if formatted_string in filtered_body or formatted_string_without_spaces in filtered_body_without_spaces:
                    return item
            except ValueError:
...
# CONFIRMATION
    @handle_api_errors
    def sending_confirmation(self, sender_email: str, sender_choice: list) -> None:
        """
        ...
        confirmation_body = auto_confirmation(workout_datetime, location)
        self.send_message(sender_email, f'Potwierdzenie - trening {workout_datetime}', confirmation_body)
        print(f'Confirmation has been sent to {sender_email}')
        self.something_happened = True
        self.file_management.remove_from_waiting_list(sender_email)

# CALENDAR
    def schedule_workout(self, sender_email: str, sender_choice: list) -> None:
        """
        ...
        try:
            for event in self.events:
                if (sender_choice[0] == event['start']['dateTime']
                    and sender_choice[1].lower() == event['summary'].lower()
                    and sender_choice[2] == event['location']):
                    event_id = event['id']
                    updated_event = self.calendar_service.events().get( # pylint:disable=maybe-no-member
                    calendarId='primary', eventId=event_id).execute()
                    updated_event['summary'] = sender_email
                    self.calendar_service.events().update( # pylint:disable=maybe-no-member
                    calendarId='primary', eventId=event_id, body=updated_event).execute()
                    print('WORKOUT HAS BEEN SCHEDULED')
                    self.sending_confirmation(sender_email, sender_choice)
                    break
    ...
    def calendar_stuff(self) -> None:
        ...
        """
        events_result = self.calendar_service.events().list( # pylint:disable=maybe-no-member
            calendarId='primary',
        ...
        if not self.events:
            ...
            print('CANNOT RESPOND TO ALL THE MAILS WITHOUT INFORMATION ABOUT AVAILABLE WORKOUTS ("wolne").')
            sys.exit("Script has been stopped. Please provide the optional workout hours (IN YOUR CALENDAR) to proceed.")
        ...
        if len(self.dates) < 15:
            print(f'There is/are OLNY {len(self.list_of_optional_hours)} optional workout hour(s) found in day(s) {min(self.dates)} - {max(self.dates)}.')
            self.logger.warning('There is/are ONLY %s optional workout hour(s) found in day(s) %s - %s.', len(self.list_of_optional_hours), min(self.dates), max(self.dates))
...
# RECIEVED MAILS
    def handle_response(self, sender_email: str) -> None:
        ...
        if sender_choice:
            self.schedule_workout(sender_email, sender_choice)
        else:
            print("I couldn't find the client's choice of a workout. You have to do it manually :(")
            print("I'm not removing the 'UNREAD' label from it so you'll see this mail and take care of it on yourself.")
            print(self.one_message_keyword_filter['body'].split('>', 1)[0].replace('\n', ''))
    ...
    def check_email_response_status(self, sender : str, receiving_time : str) -> str:
        ...
        if sender in self.file_management.sent_mails_waiting_for_answer_or_confirmation:
            sent_time = self.file_management.sent_mails_waiting_for_answer_or_confirmation[sender]
            if receiving_time_dt > sent_time:
                return 'RESPONSE'
        if sender not in self.file_management.sent_mails_waiting_for_answer_or_confirmation:
            return 'QUESTION'
        return ""
        ...
        if parts:
            for part in parts:
                ...
                if mime_type == "text/plain":
                    if data:
                        self.one_message_keyword_filter['body']= urlsafe_b64decode(data).decode()
                elif mime_type == "text/html":
                    if data:
                        self.one_message_keyword_filter['body']= html2text.HTML2Text().handle(
                    ...
                    for part_header in part_headers:
                        ...
                        if part_header_name == "Content-Disposition":
                            if "attachment" in part_header_value:
                                self.one_message_keyword_filter['attachment'] = f"There is an attachment: {part.get('filename')}, size: {self.utility.get_size_format(self, body.get('size'))}."
            return True
        else:
            return False
    ...
    def parse_headers(self, headers: List[Dict[str, str]]) -> None:
        ...
        for header in headers:
            ...
            if name == 'from':
                ...
                self.one_message_keyword_filter['date'] = self.utility.format_date(value)

    def parse_body(self, parts: List[Dict[str, Any]], payload: Dict[str, Any], mail: str) -> None:
        """
        ...
        if not self.parse_parts_with_keywords(parts, mail):
            ...
            if body_data:
                try:
                    ...
                    self.one_message_keyword_filter['body'] = html2text.HTML2Text().handle(decoded_body)
                except Exception as e:
                    self.logger.error("Error decoding message body: %s",e, exc_info=True)
...
# FIRST MAIL
    ...
    def answering_to_first_mail(self) -> None:
        """
        ...
        if self.file_management.mails_to_answer:
            ...
            for mail in self.file_management.mails_to_answer.items():
                body = auto_reply(workout_options)
                self.send_message(mail[0], f'ODP:{mail[1]}', body)
                print(f'Response has been sent to {mail[0]}')
                self.file_management.sent_mails_waiting_for_answer_or_confirmation[mail[0]] = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    ...
    def received_email_filters(self) -> None:
        ...
        response_status = self.check_email_response_status(sender_email, self.one_message_keyword_filter['date'])
