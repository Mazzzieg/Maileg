...
class UtilityFunctions:
    ...
    def day_of_a_week(self, date_time : str) -> str:
        ...
        try:
            ...
            raise ValueError(f"Invalid date or format: {date_time}. Expected format: 'YYYY-MM-DDTHH:MM:SSZ'") from e
        except Exception as e:
            self.logger.error("Unexpected error when parsing date string %s : %e", date_time, str(e))
            raise
    ...
    def format_date(self, value: str) -> str:
        ...
        try:
            if '+0000' in value:
                original_date = datetime.strptime(value, '%a, %d %b %Y %H:%M:%S %z') + timedelta(hours=2)
            else:
