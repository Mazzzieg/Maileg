...
class FileManagement:
    ...
    def creds_finder(self):
        ...
        """
        locations_to_check = ["./credentials.json", os.path.expanduser('~/Downloads/credentials.json'), os.path.expanduser(f'~/Downloads/{self.user_mail}.json'), os.path.expanduser('~/Desktop/credentials.json'),  os.path.expanduser(f'~/Desktop/{self.user_mail}.json')]
        file_found = False
        for location in locations_to_check:
            if os.path.exists(location):
                try:
                    ...
                    self.logger.error("An operating system error occurred while moving the credentials file: %e", e)
                    raise
                except Exception as e:
                    # General error handling for other exceptions
                    self.logger.error("An unexpected error occurred while moving the credentials file: %e", e)
                    raise
        if not file_found:
    ...
    def add_to_waiting_list(self) -> None:
        ...
        Update the list of emails waiting for a response.
        """
        with open(f"./users/{self.user_mail}/mails_waiting_for_answer.txt", 'a', encoding= 'UTF-8') as f:
            for customers_mail, date_of_responce in self.sent_mails_waiting_for_answer_or_confirmation.items():
                f.write(f"\n  {customers_mail} : {date_of_responce}")
                ...
                with open(file_path, 'r', encoding='UTF-8') as f:
                    for line in f:
                        ...
                        if len(parts) == 2:
                            ...
                            try:
                                date_time_obj = self.utility.string_to_datetime(date_time_str)
                                self.sent_mails_waiting_for_answer_or_confirmation[email] = date_time_obj
                            except ValueError:
                                self.logger.error("Invalid date format for 'receiving_time': %s."
                                                "Expected format is 'YYYY-MM-DDTHH:MM:SSZ'.", date_time_str)
                                raise ValueError
                        if len(parts) == 1 and not parts[0]:
                            pass
                        else:
                            self.logger.error("Invalid line format in %s: %s. "
                                            "Expected format is 'email: YYYY-MM-DDTHH:MM:SSZ'. Skipping...", file_path, line)
            except Exception as e:
                self.logger.error("An unexpected error occurred while reading the file: %s", e, exc_info=True)
                raise
